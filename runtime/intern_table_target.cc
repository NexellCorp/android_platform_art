...
#define ART_STRING_TABLE_FILE   "/system/art_string_table.bin"

void InternTable::AddImagesStringsToTable(const std::vector<gc::space::ImageSpace*>& image_spaces) {
  MutexLock mu(Thread::Current(), *Locks::intern_table_lock_);
  struct stat stat;
  if (lstat(ART_STRING_TABLE_FILE, &stat) != 0) {
load_from_scratch:
    ...
    uint8_t *ptr = (uint8_t *)malloc(strong_interns_.Size());
    if (ptr == nullptr) {
      LOG(ERROR) << "Can't alloc for intern string table buffer -> size is " << strong_interns_.Size();
    } else {
      LOG(INFO) << "Call WriteToMemory " << (unsigned long)ptr;
      size_t writeSize = strong_interns_.WriteToMemory(ptr);
      int fd = open(ART_STRING_TABLE_FILE, O_WRONLY);
      if (fd < 0) {
        LOG(ERROR) << "Can't create " << ART_STRING_TABLE_FILE;
      } else {
        size_t ret = write(fd, ptr, writeSize);
        if (ret != writeSize)
          LOG(ERROR) << "Write size is invalid";
        close(fd);
      }
      free(ptr);
    }
  } else {
    uint8_t *ptr = (uint8_t *)malloc(stat.st_size);
    if (!ptr) {
      LOG(ERROR) << "Can't alloc for string table buffer -> size is " << stat.st_size;
      goto load_from_scratch;
    }
    int fd = open(ART_STRING_TABLE_FILE, O_RDWR);
    if (fd < 0) {
      LOG(ERROR) << "Can't open " << ART_STRING_TABLE_FILE;
      free(ptr);
      goto load_from_scratch;
    } else {
      ssize_t readSize = read(fd, ptr, stat.st_size);
      if (readSize != stat.st_size)
        LOG(ERROR) << "Read size is invalid";
      strong_interns_.AddTableFromMemory(ptr);
      LOG(INFO) << "Succeed to AddTableFromMemory!!!";
      free(ptr);
    }
  }
...
size_t InternTable::Table::WriteToMemory(uint8_t* ptr) {
  LOG(INFO) << "WriteToMemory Entered: " << (unsigned long)ptr;
  if (tables_.empty()) {
    return 0;
  }
  LOG(INFO) << "WriteToMemory: " << __LINE__;
  ...
  return combined.WriteToMemory(ptr);
}
